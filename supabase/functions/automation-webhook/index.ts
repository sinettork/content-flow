...
Deno.serve(async req=>{
 ...
 const token=req.headers.get("x-contentflow-webhook-token");
 ...
 if(!e||typeof e!=="object"||typeof e.provider!=="string"||typeof e.external_account_id!=="string"||typeof e.event_type!=="string"||e.provider.length>40||e.external_account_id.length>255||e.event_type.length>100||("payload" in e&&(!e.payload||typeof e.payload!=="object"||Array.isArray(e.payload))))return json({error:"provider, external_account_id and event_type are required and must be valid strings"},400);
 ...
 try {
   ...
   if(re)throw re;
   const runs:string[]=[];
   for(const rule of rules??[]){const conditions=Array.isArray(rule.conditions)?rule.conditions:[];if(!matches(conditions,e.payload??{}))continue;const key=endpoint.workspace_id+":"+event.id+":"+rule.id;const {data:run,error:r}=await supabase.from("automation_runs").insert({workspace_id:endpoint.workspace_id,rule_id:rule.id,event_id:event.id,status:"queued",idempotency_key:key,input:e.payload??{}}).select("id").single();if(r&&r.code!=="23505")throw r;if(run)runs.push(run.id)}
   const {error:statusError}=await supabase.from("automation_events").update({status:"processed",processed_at:new Date().toISOString(),error:null}).eq("id",event.id);
   if(statusError)throw statusError;
   return json({ok:true,event_id:event.id,queued_runs:runs},202);
 } catch(error) {
   const message=error instanceof Error?error.message:String(error);
   await supabase.from("automation_events").update({status:"failed",error:message}).eq("id",event.id);
   console.error("Webhook event processing failed",{eventId:event.id,error:message});
   return json({error:"Webhook event processing failed",event_id:event.id},500);
 }
});