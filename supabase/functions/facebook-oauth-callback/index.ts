...
function redirectUrl(ok:boolean){const base=Deno.env.get("CONTENTFLOW_APP_URL");if(!base)return null;const u=new URL("/app/automation",base);u.searchParams.set("facebook",ok?"connected":"error");return u.toString();}
Deno.serve(async req=>{
 ...
 if(error||!code||!state)return Response.redirect(redirectUrl(false)??new URL("/",req.url).toString(),302);
 const hash=await sha(state);
 const {data:s,error:se}=await db.from("social_oauth_states").select("id,workspace_id,user_id,expires_at,consumed_at").eq("state_hash",hash).eq("provider","facebook").maybeSingle();
 ...
 try{
  ...
  return Response.redirect(redirectUrl(true)??new URL("/",req.url).toString(),302);
